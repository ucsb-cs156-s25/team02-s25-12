...
describe("MenuItemReviewsIndexPage tests", () => {
  ...
  test("renders three reviews correctly for regular user", async () => {
    ...
    const createMenuItemReviewsButton = screen.queryByText("Create Review");
    expect(createMenuItemReviewsButton).not.toBeInTheDocument();

    const itemId = screen.getByText("7");
    expect(itemId).toBeInTheDocument();

    const reviewerEmail = screen.getByText("simonryan@ucsb.edu");
    expect(reviewerEmail).toBeInTheDocument();

    const stars = screen.getByText("5");
    expect(stars).toBeInTheDocument();

    const comments = screen.getByText("amazing");
    expect(comments).toBeInTheDocument();

    const dateReviewed = screen.getByText("2023-01-03T00:00:00");
